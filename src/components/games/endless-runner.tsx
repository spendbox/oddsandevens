...
export default function EndlessRunner({
  ...
  const [frame, setFrame] = useState<{ y: number; obstacles: Obstacle[] }>({
    y: 0,
    obstacles: [],
  });
  ...
  useRaf(
    (dt) => {
      ...
      // Collision: overlapping boxes on both axes.
      ...
      setFrame({ y: playerY.current, obstacles: obstacles.current });
    },
    phase === "running"
  );
  ...
  const start = () => {
    ...
    setScore(0);
    setFrame({ y: 0, obstacles: [] });
    setPhase("running");
  };

  return (
    <div className="w-full">
      <Hud items={[{ label: "Distance", value: `${score} m` }]} />
      <Stage
        ratio="3 / 2"
      ...
      >
        ...
        {phase === "idle" && (
          <StartOverlay
            title="Ready to run?"
            hint="Tap the screen (or press space) to jump. One hit ends the run."
