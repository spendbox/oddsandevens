...
/**
 * Every question, given the funding ladder in force.
 *
 * It was a module-level constant, which it can no longer be: two answers here
 * print the cost of putting a box up and one of them prints the whole table of
 * it, and an admin sets those numbers now. A constant would have been last
 * deploy's price list, stated as fact, on the page whose whole rule is that
 * what it says is true of the code.
 */
export function faqItems(ladder: FundingLadder): FaqItem[] {
  ...
  return [
    ...
    },
    {
      ...
      answer: [
        `${formatNaira(MAX_FUNDING_KOBO)} is the most that can be put behind one box. That's the largest single transfer our payment provider will make, so it's also the most a winner can be paid in one go.`,
      ],
    },
